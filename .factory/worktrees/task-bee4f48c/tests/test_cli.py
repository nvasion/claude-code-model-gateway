...
class TestThirdPartyModelsCommand:
    ...
    def test_no_config_exits_zero_with_hint(self, runner, tmp_path):
        ...
        # Point GATEWAY_CONFIG at a guaranteed-nonexistent path so
        # find_config_file() returns None regardless of the project root.
        # (find_config_file returns None when the env-var path doesn't exist.)
        nonexistent = str(tmp_path / "no_such_gateway.yaml")
        result = runner.invoke(
            main,
            ["third-party-models"],
            env={"GATEWAY_CONFIG": nonexistent},
