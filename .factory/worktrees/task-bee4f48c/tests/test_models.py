...
class TestGatewayConfig:
    ...
    # ------------------------------------------------------------------
    # resolve_model / model_aliases tests
    # ------------------------------------------------------------------

    def test_resolve_model_returns_alias_target(self):
        """resolve_model returns the target when an alias exists."""
        config = GatewayConfig(
            model_aliases={
        ...
        assert (
            config.resolve_model("claude-sonnet-4-20250514")
            == "arcee-ai/trinity-large-preview:free"
        )

    def test_resolve_model_passthrough_for_unknown(self):
        """resolve_model returns the input unchanged when no alias exists."""
        config = GatewayConfig(
            model_aliases={"some-alias": "some-target"}
        )
        assert config.resolve_model("unknown-model") == "unknown-model"

    def test_resolve_model_empty_aliases(self):
        """resolve_model returns the input unchanged when aliases dict is empty."""
        config = GatewayConfig()
        assert config.resolve_model("claude-sonnet-4-20250514") == "claude-sonnet-4-20250514"

    def test_model_aliases_in_to_dict(self):
        """model_aliases is serialized when non-empty."""
        config = GatewayConfig(
            model_aliases={"alias-a": "real-a", "alias-b": "real-b"}
        )
        data = config.to_dict()
        assert data["model_aliases"] == {"alias-a": "real-a", "alias-b": "real-b"}

    def test_model_aliases_omitted_from_to_dict_when_empty(self):
        """model_aliases is not included in to_dict when it is empty."""
        config = GatewayConfig()
        data = config.to_dict()
        assert "model_aliases" not in data

    def test_model_aliases_roundtrip(self):
        """model_aliases survives a to_dict / from_dict round-trip."""
        original = GatewayConfig(
            model_aliases={
                "claude-sonnet-4-20250514": "arcee-ai/trinity-large-preview:free",
    ...
    def test_from_dict_model_aliases_defaults_to_empty(self):
        """from_dict produces an empty model_aliases when the key is absent."""
        config = GatewayConfig.from_dict({"default_provider": "openai"})
        assert config.model_aliases == {}

    def test_from_dict_model_aliases_parsed(self):
        """from_dict correctly parses a model_aliases mapping."""
        data = {
            "model_aliases": {
        ...
        config = GatewayConfig.from_dict(data)
        assert config.model_aliases == {"incoming-model": "outgoing-model"}
        assert config.resolve_model("incoming-model") == "outgoing-model"
