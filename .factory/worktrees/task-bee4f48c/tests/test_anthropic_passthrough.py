...
# ------------------------------------------------------------------ #
# Mock helpers for alias / header tests
# ------------------------------------------------------------------ #


class _BodyCapturingHandler(http.server.BaseHTTPRequestHandler):
    """Mock provider that records the most recent request body (as a dict)."""
    ...
    def do_POST(self):
        ...
        try:
            _BodyCapturingHandler.last_body = json.loads(raw.decode())
        except json.JSONDecodeError:
            _BodyCapturingHandler.last_body = {}
        ...
        resp = json.dumps(
            {
                ...
                "model": _BodyCapturingHandler.last_body.get("model", ""),
                "stop_reason": "end_turn",
...
class _HeaderCapturingHandler(http.server.BaseHTTPRequestHandler):
    """Mock provider that records headers from the most recent request."""
    ...
    def do_POST(self):
        length = int(self.headers.get("Content-Length", 0))
        self.rfile.read(length)
        _HeaderCapturingHandler.last_headers = dict(self.headers)
...
def _build_alias_config(api_base: str) -> GatewayConfig:
    """Build a GatewayConfig with model_aliases pointing at the body-capturing mock."""
    from src.models import AuthType, ModelConfig, ProviderConfig
    ...
    provider = ProviderConfig(
        ...
        models={
            "arcee-ai/trinity-large-preview:free": ModelConfig(
                name="arcee-ai/trinity-large-preview:free",
                display_name="Arcee Trinity (free)",
            ),
        },
    )
    return GatewayConfig(
        ...
        model_aliases={
            ...
            "claude-haiku-3-5-20241022": "arcee-ai/trinity-large-preview:free",
        },
    )


# ------------------------------------------------------------------ #
# Tests: Model alias remapping
# ------------------------------------------------------------------ #


class TestModelAliasRemapping:
    """Tests for transparent model name remapping via model_aliases."""

    @pytest.fixture(scope="class")
    def capturing_provider(self):
        """Body-capturing mock upstream provider."""
        _BodyCapturingHandler.last_body = {}
        server = ThreadedGatewayServer(("127.0.0.1", 0), _BodyCapturingHandler)
        t = threading.Thread(target=server.serve_forever, daemon=True)
        t.start()
        yield server
        server.shutdown()
    ...
    @pytest.fixture(scope="class")
    def alias_gateway(self, capturing_provider):
        """Gateway configured with model_aliases, pointed at the capturing provider."""
        host, port = capturing_provider.server_address
        config = _build_alias_config(f"http://{host}:{port}/v1")
        server, _ = run_passthrough_in_thread(
            host="127.0.0.1", port=0, gateway_config=config
        )
        yield server
        server.shutdown()
    ...
    def test_alias_model_is_rewritten_in_request_body(
        self, alias_gateway, capturing_provider
    ):
        """A request with an aliased model name has the model field rewritten upstream."""
        _BodyCapturingHandler.last_body = {}
        port = alias_gateway.server_address[1]
        _make_messages_request(port, model="claude-sonnet-4-20250514")

        received_model = _BodyCapturingHandler.last_body.get("model")
        assert received_model == "arcee-ai/trinity-large-preview:free", (
            f"Expected upstream to receive the aliased model, got {received_model!r}"
        )

    def test_alias_model_rewrite_returns_200(self, alias_gateway):
        """A request whose model name is aliased still returns HTTP 200."""
        port = alias_gateway.server_address[1]
        resp = _make_messages_request(port, model="claude-sonnet-4-20250514")
        assert resp.status == 200

    def test_multiple_aliases_all_rewritten(self, alias_gateway, capturing_provider):
        """Each configured alias is rewritten to its target model name."""
        for alias_in, expected_out in [
            ("claude-sonnet-4-20250514", "arcee-ai/trinity-large-preview:free"),
            ("claude-haiku-3-5-20241022", "arcee-ai/trinity-large-preview:free"),
        ]:
            _BodyCapturingHandler.last_body = {}
            port = alias_gateway.server_address[1]
            _make_messages_request(port, model=alias_in)
            assert _BodyCapturingHandler.last_body.get("model") == expected_out, (
                f"Alias {alias_in!r} should map to {expected_out!r}"
            )

    def test_content_length_matches_rewritten_body(
        self, alias_gateway, capturing_provider
    ):
        """Content-Length header equals the byte length of the rewritten body."""
        _BodyCapturingHandler.last_body = {}
        port = alias_gateway.server_address[1]
        _make_messages_request(port, model="claude-sonnet-4-20250514")

        # The capturing handler received and decoded the body correctly, which
        # means Content-Length matched the actual body length (otherwise
        # http.client would have raised an error reading it).
        assert _BodyCapturingHandler.last_body.get("model") == "arcee-ai/trinity-large-preview:free"

    def test_non_aliased_model_passes_through_unchanged(
        self, alias_gateway, capturing_provider
    ):
        """A model name that is not in model_aliases is forwarded unchanged."""
        _BodyCapturingHandler.last_body = {}
        port = alias_gateway.server_address[1]
        # "arcee-ai/trinity-large-preview:free" is the real model name (not aliased)
        _make_messages_request(port, model="arcee-ai/trinity-large-preview:free")
        assert (
            _BodyCapturingHandler.last_body.get("model")
            == "arcee-ai/trinity-large-preview:free"
        )


# ------------------------------------------------------------------ #
# Tests: /v1/models includes aliased model names
# ------------------------------------------------------------------ #


class TestModelAliasInModelsEndpoint:
    """Tests that /v1/models exposes alias names from model_aliases."""

    @pytest.fixture(scope="class")
    def alias_models_gateway(self):
        """Gateway with model_aliases but no real upstream needed (models are local)."""
        config = _build_alias_config("http://127.0.0.1:1")  # dummy, not contacted
        server, _ = run_passthrough_in_thread(
            host="127.0.0.1", port=0, gateway_config=config
        )
        yield server
    ...
    def test_alias_names_appear_in_models_list(self, alias_models_gateway):
        """GET /v1/models includes all configured alias names."""
        port = alias_models_gateway.server_address[1]
        resp = _gateway_request(port, path="/v1/models", method="GET")
        data = json.loads(resp.read().decode())
        ids = {m["id"] for m in data["data"]}
    ...
    def test_provider_models_still_in_list(self, alias_models_gateway):
        """Native provider model IDs are also present alongside aliases."""
        port = alias_models_gateway.server_address[1]
        resp = _gateway_request(port, path="/v1/models", method="GET")
        data = json.loads(resp.read().decode())
        ids = {m["id"] for m in data["data"]}
    ...
    def test_alias_display_name_shows_target(self, alias_models_gateway):
        """Alias model entries have a display_name that references the target."""
        port = alias_models_gateway.server_address[1]
        resp = _gateway_request(port, path="/v1/models", method="GET")
        data = json.loads(resp.read().decode())

        alias_entry = next(
            (m for m in data["data"] if m["id"] == "claude-sonnet-4-20250514"), None
        )
        assert alias_entry is not None
        assert "arcee-ai/trinity-large-preview:free" in alias_entry["display_name"]

    def test_alias_not_duplicated_if_same_as_provider_model(self):
        """An alias whose name matches a real provider model ID is not duplicated."""
        from src.models import AuthType, GatewayConfig, ModelConfig, ProviderConfig

        # alias key deliberately matches a provider model name — shouldn't duplicate
        provider = ProviderConfig(
            name="p",
            api_base="http://127.0.0.1:1/v1",
            auth_type=AuthType.NONE,
            models={"real-model": ModelConfig(name="real-model")},
        )
        config = GatewayConfig(
            default_provider="p",
            providers={"p": provider},
            model_aliases={"real-model": "some-other-model"},
        )
        server, _ = run_passthrough_in_thread(
            host="127.0.0.1", port=0, gateway_config=config
        )
        ...
        try:
            resp = _gateway_request(port, path="/v1/models", method="GET")
            data = json.loads(resp.read().decode())
            real_model_entries = [m for m in data["data"] if m["id"] == "real-model"]
...
# ------------------------------------------------------------------ #
# Tests: anthropic-* header forwarding in provider/config mode
# ------------------------------------------------------------------ #


class TestAnthropicHeaderForwarding:
    """Tests that anthropic-version and anthropic-beta are forwarded to providers."""

    @pytest.fixture(scope="class")
    def header_provider(self):
        """Header-capturing mock upstream provider."""
        _HeaderCapturingHandler.last_headers = {}
        server = ThreadedGatewayServer(("127.0.0.1", 0), _HeaderCapturingHandler)
        t = threading.Thread(target=server.serve_forever, daemon=True)
        t.start()
        yield server
        server.shutdown()
    ...
    @pytest.fixture(scope="class")
    def header_gateway(self, header_provider):
        """Gateway in config mode, routed to the header-capturing provider."""
        from src.models import AuthType, GatewayConfig, ModelConfig, ProviderConfig

        host, port = header_provider.server_address
        provider = ProviderConfig(
            name="p",
            api_base=f"http://{host}:{port}/v1",
            auth_type=AuthType.NONE,
            models={"test-model": ModelConfig(name="test-model")},
        )
        config = GatewayConfig(
            default_provider="p",
            providers={"p": provider},
    ...
    def test_anthropic_version_forwarded(self, header_gateway, header_provider):
        """anthropic-version header from the client is forwarded to the provider."""
        _HeaderCapturingHandler.last_headers = {}
        port = header_gateway.server_address[1]
        _make_messages_request(
            port,
            model="test-model",
            extra_headers={"anthropic-version": "2023-06-01"},
        )
        received = _HeaderCapturingHandler.last_headers
        assert "anthropic-version" in {k.lower(): v for k, v in received.items()}, (
            f"anthropic-version not in {list(received.keys())}"
        )

    def test_anthropic_beta_forwarded(self, header_gateway, header_provider):
        """anthropic-beta header from the client is forwarded to the provider."""
        _HeaderCapturingHandler.last_headers = {}
        port = header_gateway.server_address[1]
        _make_messages_request(
            port,
        ...
        received = {k.lower(): v for k, v in _HeaderCapturingHandler.last_headers.items()}
        assert "anthropic-beta" in received, (
            f"anthropic-beta not in {list(received.keys())}"
        )
        assert received["anthropic-beta"] == "interleaved-thinking-2025-05-14"

    def test_x_api_key_not_forwarded(self, header_gateway, header_provider):
        """x-api-key from the client is NOT forwarded (it is a managed header)."""
        _HeaderCapturingHandler.last_headers = {}
        port = header_gateway.server_address[1]
        _make_messages_request(
            port,
            model="test-model",
            extra_headers={"x-api-key": "client-secret"},
        )
        received = {k.lower(): v for k, v in _HeaderCapturingHandler.last_headers.items()}
        # The provider should not see the raw client API key
        assert received.get("x-api-key", "") != "client-secret"

    def test_authorization_not_forwarded(self, header_gateway, header_provider):
        """authorization header from the client is NOT forwarded (managed header)."""
        _HeaderCapturingHandler.last_headers = {}
        port = header_gateway.server_address[1]
        _make_messages_request(
            port,
            model="test-model",
            extra_headers={"Authorization": "Bearer client-token"},
        )
        received = {k.lower(): v for k, v in _HeaderCapturingHandler.last_headers.items()}
        assert received.get("authorization", "") != "Bearer client-token"


# ------------------------------------------------------------------ #
# Tests: Streaming uses read1 for real-time delivery
# ------------------------------------------------------------------ #


class TestStreamingRead1:
    """Tests that _relay_streaming_response uses read1 for per-chunk forwarding."""

    def _make_handler(self):
        """Return a bare AnthropicPassthroughHandler suitable for unit testing."""
    ...
    def test_relay_streaming_calls_read1_not_read(self):
        """_relay_streaming_response calls read1 instead of read on the upstream response."""
        from unittest.mock import MagicMock
        ...
        mock_resp.reason = "OK"
        mock_resp.getheaders.return_value = [("Content-Type", "text/event-stream")]
        # Return two chunks then an empty bytes to signal EOF
        mock_resp.read1.side_effect = [b"event: ping\n\n", b"event: pong\n\n", b""]

        handler._relay_streaming_response(mock_resp)

        assert mock_resp.read1.call_count == 3
        mock_resp.read.assert_not_called()

    def test_relay_streaming_writes_each_chunk(self):
        """Each non-empty chunk returned by read1 is written to wfile."""
        from unittest.mock import MagicMock
        ...
        mock_resp.reason = "OK"
        mock_resp.getheaders.return_value = [("Content-Type", "text/event-stream")]
        mock_resp.read1.side_effect = [b"chunk-a", b"chunk-b", b"chunk-c", b""]

        handler._relay_streaming_response(mock_resp)

        assert handler.wfile.write.call_count == 3

    def test_relay_streaming_flushes_after_each_chunk(self):
        """wfile.flush() is called after each chunk write."""
        ...
        mock_resp.read1.side_effect = [b"event: delta\n\n", b""]

        handler._relay_streaming_response(mock_resp)

        # flush must be called at least once
