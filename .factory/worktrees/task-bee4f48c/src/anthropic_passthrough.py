...
class AnthropicPassthroughHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle_request(self) -> None:
        ...
        # Apply model alias remapping — rewrite the model name in the request
        # body so the upstream provider receives the configured target name.
        # This lets Claude Code send canonical Claude model IDs (e.g.
        # ``claude-sonnet-4-20250514``) that the gateway transparently maps to
        # provider-specific IDs (e.g. ``arcee-ai/trinity-large-preview:free``).
        if request_model and self.gateway_config is not None:
            resolved_model = self.gateway_config.resolve_model(request_model)
            if resolved_model != request_model:
                logger.info(
                    "Remapping model %s -> %s",
                    request_model,
                    resolved_model,
        ...
        if body is not None:
            upstream_headers["Content-Type"] = self.headers.get(
    ...
    def _forward_with_retry(
        ...
        try:
            ...
            logger.error("Error forwarding %s %s: %s", self.command, self.path, exc)
            self._send_error(
                502,
                {
                    "type": "error",
    ...
    def _relay_standard_response(self, upstream_resp: http.client.HTTPResponse) -> None:
    ...
    def _relay_streaming_response(
        ...
        # read1() returns data from at most one underlying HTTP chunk so each
        # SSE event is forwarded immediately rather than being buffered until a
        # fixed byte count is accumulated (which is what read(N) does under
        # chunked transfer-encoding).
        try:
            while True:
                chunk = upstream_resp.read1(4096)
                if not chunk:
                    break
    ...
    def _handle_gateway_error(self, exc: GatewayError, request_start: float) -> None:
    ...
    def _serve_models_from_config(self) -> None:
        ...
        # Also expose alias names so Claude Code's /model picker shows
        # familiar Claude model IDs alongside native provider model IDs.
        # Aliases are added only if the alias name is not already in the list.
        existing_ids = {m["id"] for m in models_data}
        for alias_name, target_name in self.gateway_config.model_aliases.items():
            if alias_name not in existing_ids:
                models_data.append(
                    {
                        "type": "model",
                        "id": alias_name,
                        "display_name": f"{alias_name} \u2192 {target_name}",
                        "created_at": "2024-01-01T00:00:00Z",
                    }
                )
    ...
    def _parse_api_base(self, api_base: str) -> Optional[tuple[str, int, bool, str]]:
    ...
    @staticmethod
    def _should_forward_client_header(lower_key: str) -> bool:
        """Return True if a client request header should be forwarded upstream.

        Forwards ``user-agent``, ``accept*``, and ``anthropic-*`` headers.
        The ``anthropic-version`` and ``anthropic-beta`` headers are included
        so that API features like extended thinking and prompt caching work on
        Anthropic-compatible upstreams (e.g. OpenRouter).

        Args:
            lower_key: The header name already lowercased.

        Returns:
            True when the header should be included in the upstream request.
        """
        return (
            lower_key == "user-agent"
            or lower_key.startswith("accept")
            or lower_key.startswith("anthropic-")
        )
    ...
    def _build_provider_upstream_headers(
        ...
        for key, val in self.headers.items():
            ...
            if lower_key in MANAGED_HEADERS:
                continue
            if self._should_forward_client_header(lower_key):
                headers[key] = val
...
def run_passthrough(
    ...
    logger.info(
        "Gateway listening on %s:%d " "(upstream: %s, version: %s, max_retries: %d)",
        host,
        port,
        upstream_label,
        anthropic_version,
