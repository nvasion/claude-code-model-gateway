...
class GatewayConfig:
    ...
    Attributes:
        ...
        model_aliases: Map of incoming model names to upstream model names.
            Allows Claude Code's fixed model names (e.g. ``claude-sonnet-4-20250514``)
            to be transparently rewritten to provider-specific IDs before
            forwarding (e.g. ``arcee-ai/trinity-large-preview:free``).
    ...
    def resolve_model(self, model_name: str) -> str:
        """Resolve a model alias to the actual upstream model name.

        Returns the aliased (target) name when *model_name* appears in
        ``model_aliases``, otherwise returns *model_name* unchanged.

        Args:
            model_name: The model ID as received from the client.

        Returns:
            The upstream model ID to use.
        """
